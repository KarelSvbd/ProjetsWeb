...
function returnResponse() {
    ...
    //Cette partie est uniquement accessible si l'utilisateur est connecté et s'il possède une clé de session
    else if(isset($_GET["key"]) && isset($_GET["idUser"]))
    {
        if(verifyIfKeyExistWithTheRightUser($_GET["key"], $_GET["idUser"]))
        {
            ...
            if(isset($_GET["personnes"])){
                ...
                switch($_GET["personnes"]){
                    case "all":
                        ...
                        //Différents points d'entrée de personnes
            ...
            http_response_code(401);
                    $response = array(
                        "401" => "Vous n'avez pas accès à cette partie ou votre requête n'est pas valide. Veuillez vous connecter.",
                    );
        }
    }
